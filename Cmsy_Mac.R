...
setwd("~/Library/CloudStorage/GoogleDrive-siskaagustina03@gmail.com/.shortcut-targets-by-id/1-RVmNqo13M9irz7Ii4zVJ-Y919XdUCsP/YKAN Working File/Training-Analisis Data Perikanan/Training SDI/cMSY/cMSY-Mac")

#-----------------------------------------
# Required settings, File names ----
#-----------------------------------------
catch_file  <- "Stocks_Catch_INA_Scads_poisson.csv" #"Stocks_Catch_2020CMSYrun2_v5_RS - Copy.csv"  #"CombStocks_Catch_2020CMSYrun3_v4.csv"  # "SAUP_Catch_1.csv"  #"SimCatchCPUE_4.csv"  # "Stocks_Catch_Aust_2.csv" #"STECF_Catch_2020_2.csv" #"tRFMO_Catch_2020.csv" #"ICES_Catch_2020.csv" #"Global_Stocks_Catch.csv" #"SimCatchCPUE_4.csv" #"Stocks_Catch_Test.csv"  #"Stock_Catch_forRainer.csv" # "SimCatchCPUE_3.csv" #  name of file containing "Stock", "yr", "ct", and optional "bt"
id_file     <- "Stocks_ID_INA_Scads_poisson.csv" #"Stocks_ID_forDeng_RSb - Copy.csv" # "CombStocks_ID_2020CMSYrun3_v3_RF3.csv"   #    "SAUP_ID_2.csv"    #"SimSpecCPUE_4_NA_int_end.csv"  #"Train_ID_7j.csv" # "Stocks_ID_Aust_4.csv"  #"STECF_ID_2020_2.csv" #tRFMO_ID_2020_2.csv" #"ICES_ID_2020_4.csv" #"Robust_Stocks_ID_10_allNA.csv" #"SimSpecCPUE_4.csv"#"Stocks_ID_R_7.csv"  #"Stock_ID_forRainer.csv"  #  "NCod_ID_4.csv" #"SimSpecCPUE_3.csv" #  name of file containing stock-specific info and settings for the analysis
...
# If the line below is commented out (#), all stocks in the input file will be analyzed
 stocks <-"layang" #"Blacknose shark - Atlantic"  #"Alop_sup_Indian"  #"Acadian redfish - Gulf of Maine / Georges Bank"# "Hogfish - Florida Keys / East Florida"  #"Splitnose rockfish - Pacific Coast"  #"Acadian_redfish"  # "ple.27.7d" #"cod.27.1-2coast"#"cod.27.7e-k" # "Acadian redfish - Gulf of Maine / Georges Bank"  #c("Greenspotted rockfish - Pacific Coast")
...
write.rdata  <- D #><>HW write R data file
...
# Read data
cdat         <- read.csv2(catch_file, header=T, dec=".", stringsAsFactors = FALSE)
cinfo        <- read.csv2(id_file, header=T, dec=".", stringsAsFactors = FALSE)
...
for(stock in stocks) {
  ...
  #-----------------------------------------------------------------
  # Check for operating system, open separate window for graphs if macOS
  if (tolower(Sys.info()['sysname']) == "darwin") {
    # Jika sistem operasi adalah macOS, buka jendela grafis
 ...
 #   bt           <- bt.raw
  ...
  if(mgraphs==T) {
    ...
    if (tolower(Sys.info()['sysname']) == "darwin" || tolower(Sys.info()['sysname']) == "linux") {
      # Jika sistem operasi adalah macOS atau Linux, buka jendela grafis dengan quartz()
      quartz(width = 14, height = 12)
    } else {
      # Jika sistem operasi adalah Windows, buka jendela grafis dengan windows()
      quartz(width = 14, height = 12)
    }
  ...
  if(pp.plot==T) {
    # open window for plot of four panels
    # Check for operating system and open separate window for graphs if macOS
    if (tolower(Sys.info()['sysname']) == "darwin") {
      # Jika sistem operasi adalah macOS, buka jendela grafis dengan quartz() dan atur lebar dan tinggi jendela
      quartz(width = 17, height = 12)
    } else {
      # Jika sistem operasi adalah Windows atau sistem lainnya, lanjutkan tanpa membuka jendela grafis tambahan
    }
    # Atur margin menjadi lebih sempit
  ...
  if(kobe.plot==T){
    ...
    # Atur parameter grafis untuk satu plot saja
    par(mfrow = c(1, 1))
    # make margins narrower
  ...
  years.retrospective[[retrosp.step+1]]<-yr #retrospective analysis
	...
	if (retros == T){
	   ...
	   if(grepl("win",tolower(Sys.info()['sysname']))) {quartz(width = 14, height = 7)}
